...
import DetailedCanvas from '@/components/admin/detailedCanvas'
import { CanvasProvider } from '@/providers/canvas_providers/canvasContextProvider'
import DetailedCanvasModal from '@/components/admin/detailedCanvasModal'
...
export default function AdminDashboardOrderDetails() {
  ...
  const flashing = orderData.flashings.find((flash) => flash.id === flashingId)?.moreDetails
            ...
            <div className="bg-white rounded-lg col-span-2">
              ...
              <div className="flex flex-col items-center justify-center">
                <div className="relative flex flex-col items-center justify-center p-6 w-full">
                  ...
                  <PreviewCanvas flashing={flashing} view={canvasView} height={256} />
                  <div className="flex items-end justify-between w-full">
                    <span className="text-[14px]/[17px] self-start">Unit: mm</span>
                    <Maximize
                      className="cursor-pointer"
                      onClick={() => setShowDetailedCanvas(true)}
                    />
                  </div>
                </div>
                  ...
                  <div className="border border-border-default rounded-md flex flex-col p-4 gap-3">
                    ...
                    {orderData.flashings.find((flash) => flash.id === flashingId)?.moreDetails
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="body-regular font-bold">
                          {
                            orderData.flashings.find((flash) => flash.id === flashingId)
                              ?.moreDetails.color?.name
                          }
                        </span>
                      </div>
                    ) : (
                      <div className="flex items-center justify-between">
                        ...
                        <span className="body-regular font-bold">
                          {
                            orderData.flashings.find((flash) => flash.id === flashingId)
                              ?.moreDetails.thickness?.thickness
                          }{' '}
                          mm
                        </span>
                      </div>
            ...
            <div className="bg-white rounded-lg flex flex-col p-6 gap-3">
              ...
              <div className="flex items-start justify-start gap-1">
                ...
                <span className="body-regular">
                  {orderData.pickupInfo.address.streetAddress},{' '}
                  {orderData.pickupInfo.address.suburb}, {orderData.pickupInfo.address.state}{' '}
                  {orderData.pickupInfo.address.postcode}
                </span>
              </div>
              <div className="flex items-start justify-start gap-1">
                <ProfileNav className="size-5" />
                <span className="body-regular">
                  {orderData.recipientInfo.recipientName} -{' '}
                  {orderData.recipientInfo.recipientMobile}
                </span>
              </div>
            </div>
          </div>
