...
const formSchema = z.object({
  ...
  jobReferenceCode: z.string().nonempty('Job reference code is required'),
...
export default function JobReferencesPage() {
  ...
  const onSubmit = (data: FormValues) => {
    ...
    router.push(`/dashboard/job-references/add/recipient`)
      ...
      <header className="fixed top-0 left-0 w-full h-14 flex items-center justify-center z-10 bg-white border-b-1 border-border-dark">
        <div className="flex items-center justify-between h-full w-full px-4">
          <div className="flex items-center gap-[18px] pr-3">
            <Link href={`/dashboard/job-references/add`}>
              <ArrowLeft />
            </Link>
      ...
      <div className="overflow-scroll h-full pt-18 pb-22 px-4 no-scrollbar">
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
          <LabeledInput
            ...
            defaultValue={newJobReference.streetAddress}
          ...
          <LabeledInput
            ...
            defaultValue={newJobReference.suburb}
            {...register('suburb')}
          />
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Select
                  ...
                  // defaultValue={newJobReference.state}
                />
              )}
            />
          </div>
          <LabeledInput
            ...
            defaultValue={newJobReference.postcode}
            {...register('postcode')}
          />
          ...
          <LabeledInput
            ...
            defaultValue={newJobReference.addressTitle}
            {...register('addressTitle')}
          />

          <input
            type="hidden"
            value={newJobReference.jobReferenceCode}
            {...register('jobReferenceCode')}
          />
          <input type="hidden" value={newJobReference.projectName} {...register('projectName')} />
