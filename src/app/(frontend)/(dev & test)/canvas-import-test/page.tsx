...
function createColorSideCrushFoldObject(circle, direction, position) {
  const dirCoef = direction ? 1 : -1
  ...
  if (position === 'start') {
    ...
    crushFoldObject = new Circle({
      radius: 10,
      left: cx - 11.25 * Math.cos(mainAngle - 90 * dirCoef) - 4.25 * Math.cos(mainAngle),
      top: cy - 11.25 * Math.sin(mainAngle - 90 * dirCoef) - 4.25 * Math.sin(mainAngle),
      angle: 0,
      startAngle: (mainAngle * 180) / Math.PI + 90,
      endAngle: (mainAngle * 180) / Math.PI - 90,
      stroke: '#ef9393',
  ...
  circles.map((circle) => {
    if (!circle.line1 && circle.line2) {
      ...
      if (!circle.line1.circle1.line1) {
        ...
        if (startCrushFold) {
          const crushFoldObject = createColorSideCrushFoldObject(startCircle, direction, 'start')
      ...
      if (endCrushFold) {
        ...
        const crushFoldObject = createColorSideCrushFoldObject(endCircle, direction, 'end')
