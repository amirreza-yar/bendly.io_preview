...
export default function VerifyNewMobilePage() {
  const router = useRouter()

  const { user } = useUser()
  const mobile = user.mobile

  const newMobile = useSearchParams().get('mobile') ?? ''

  // if (!newMobile) notFound()
  ...
  const onSubmitVerifyMobile = async (data: VerifyPhoneOTPValue) => {
    ...
    // }
    // console.log(res)
    router.push(`/dashboard/account?mobile=${newMobile}`)
  }

  const handleResendMobileCode = async () => {
  ...
  return (
    <>
      <Header
        title="Verify New Mobile"
        returnHref="/dashboard/account/verified-mobile"
        className="border-0"
      />
      ...
      <ContentWrapper className="pt-24">
        <div className="grid gap-6 items-center">
          <div className="grid items-center text-center gap-2">
            <h5>Verify Your New Mobile</h5>
            <p className="subtitle-regular">We've sent a 5-digit code to your new mobile number:</p>
          ...
          <div className="flex gap-2 items-center justify-center">
            ...
            <CodeResendTime ref={resendRef} onResendHandler={handleResendMobileCode} />
          </div>
        </div>
      </ContentWrapper>
    </>
