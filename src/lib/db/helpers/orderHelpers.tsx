...
export async function deleteFlashingFromOrderByIds(
  ...
  try {
    ...
    const modifiedCount = await db.transaction(
      ...
      async () => {
        ...
        const count = await db.orders
          ...
          .modify((order: Partial<StoredOrder> & { flashings?: StoredOrderFlashing[] }) => {
            if (!Array.isArray(order.flashings) || order.flashings.length === 0) return
            const before = order.flashings.length
            order.flashings = order.flashings.filter((f) => f.id !== flashingId)
    ...
    console.error('removeFlashingFromOrder transaction failed', { orderId, flashingId }, err)
    throw err
  }
}
