...
import { OrderProgress } from '@/types/orders/orderType'
...
interface OrderProgressionObjectProps {
  progress: OrderProgress
}

export function ProgressionObject({ progress }: OrderProgressionObjectProps) {
  const progressItem = orderProgresses.find((prog) => prog.progress === progress)
      ...
      <div className="relative mx-2 grid">
        <div className="border-l border-border-info">
          <div className="mb-2 ms-6">
            ...
            <p className="body-small text-subtitle">Your order has been received </p>
          </div>
        </div>
        ...
        <div className="ms-6">
          ...
          <p className="body-small text-subtitle">Order delivered / picked up successfully</p>
        </div>
      </div>
    )
  } else {
    return (
      <div className="grid mx-2">
        {orderProgresses.map((prog) => (
          <div
            key={prog.index}
            className={cn('relative', prog.index !== 4 && 'border-l border-border-info')}
          >
...
export function RejectedProgressionObject({ progress }: OrderProgressionObjectProps) {
  const progressItem = orderProgresses.find((prog) => prog.progress === progress)
  const progressIndex = progressItem?.index ?? -1
    ...
    <div className="grid mx-2">
      ...
      <div key={progressIndex} className="relative">
        <div className="ms-6">
          ...
          <p className="body-small text-subtitle">Your request was not approved</p>
        </div>
      </div>
    </div>
  )
...
export function RequestProgressionObject({ progress }: RequestProgressionObjectProps) {
  const progressItem = requestProgresses.find((prog) => prog.progress === progress)
  const progressIndex = progressItem?.index ?? -1
...
export function RejectedRequestProgressionObject({ progress }: RequestProgressionObjectProps) {
  const progressItem = requestProgresses.find((prog) => prog.progress === progress)
  const progressIndex = progressItem?.index ?? -1
