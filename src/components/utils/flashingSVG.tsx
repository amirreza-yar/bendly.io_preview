...
function calculateLines(nodes: StoredFlashing['nodes']) {
  ...
  const longestLine = lines.reduce((max, l) => (l.length > max.length ? l : max), lines[0])
...
function getNodeBounds(nodes: StoredFlashing['nodes'], offset: number, path3DOffset: number) {
  if (!nodes.length) {
...
function genStartCrushFoldCoor(
  ...
  const A2 = {
    left:
      A1.left +
      crushFoldOffset * Math.cos(((lines[0].angle + 90 * (crushFoldDir ? 1 : -1)) * Math.PI) / 180),
    top:
      A1.top +
      crushFoldOffset * Math.sin(((lines[0].angle + 90 * (crushFoldDir ? 1 : -1)) * Math.PI) / 180),
  }
  ...
  const A4 = {
    left: A3.left + (crushFoldOffset / 2) * Math.cos((lines[0].angle * Math.PI) / 180),
    top: A3.top + (crushFoldOffset / 2) * Math.sin((lines[0].angle * Math.PI) / 180),
  }
...
function genEndCrushFoldCoor(
  ...
  const A1 = {
    left: M.left + crushFoldOffset * Math.cos((lines[lines.length - 1].angle * Math.PI) / 180),
    top: M.top + crushFoldOffset * Math.sin((lines[lines.length - 1].angle * Math.PI) / 180),
  }
  ...
  const A2 = {
    left:
      A1.left +
      crushFoldOffset *
        Math.cos(((lines[lines.length - 1].angle + 90 * (crushFoldDir ? 1 : -1)) * Math.PI) / 180),
    top:
      A1.top +
      crushFoldOffset *
        Math.sin(((lines[lines.length - 1].angle + 90 * (crushFoldDir ? 1 : -1)) * Math.PI) / 180),
  }
  ...
  const A4 = {
    left:
      A3.left - (crushFoldOffset / 2) * Math.cos((lines[lines.length - 1].angle * Math.PI) / 180),
    top: A3.top - (crushFoldOffset / 2) * Math.sin((lines[lines.length - 1].angle * Math.PI) / 180),
  }
    ...
    <>
      ...
      >
        ...
        {flashing.startCrushFold && (
          <path
            d={genStartCrushFoldCoor(flashing.nodes, crushFoldOffset, flashing.crushFoldDir)}
            stroke="#262626"
            fill="none"
            strokeWidth={strokeWidth}
            strokeLinecap="round"
        ...
        {flashing.endCrushFold && (
          <path
            d={genEndCrushFoldCoor(flashing.nodes, crushFoldOffset, flashing.crushFoldDir)}
            stroke="#262626"
            fill="none"
            strokeWidth={strokeWidth}
            strokeLinecap="round"
