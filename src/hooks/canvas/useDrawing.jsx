...
export default function useDrawing() {
  ...
  const changeDrawingDirectionOnCrushFold = () => {
    console.log('crush object clicked')
  ...
  useEffect(() => {
    ...
    const onDrawing = (event) => {
      ...
      console.log('prevCircle ', activeCircle)
      ...
      const circleExisting = canvas
        .getObjects()
        .find((obj) => obj.type === 'circle' && obj.left === x && obj.top === y)
    ...
    if (isDrawing && !isMoving) {
      ...
      canvas
        .getObjects()
        .filter((o) => o.type === 'circle' && !o.bufferCircle && !o.isBufferCircle)
        .forEach((circle) => {
          circle.set({
            // selectable: true,
            lockScalingX: true,
      ...
      if (startCrushFoldObjectRef.current || endCrushFoldObjectRef.current) {
        ...
        } else if (!startCrushFoldObjectRef.current && endCrushFoldObjectRef.current) {
          circles.forEach((cir) => {
            if (cir.line2 && !cir.line1) {
              clickedCircle = cir
            } else if (!cir.line2 && cir.line1) {
        ...
        if (otherCrushFoldObject && otherSideCircle) {
          ...
          otherCrushFoldObject?.on('mousedown', changeDrawingDirectionOnCrushFold)
        } else if (startCrushFoldObjectRef.current && endCrushFoldObjectRef.current) {
          canvas.on('mouse:down', () => {
            document.getElementById('trigger-both-ends-closed-crush-fold-alert-dialog').click()
          })
        }
      } else {
        if (activeCircle.current) {
