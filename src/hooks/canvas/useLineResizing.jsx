...
export default function useLineResizing() {
  ...
  const createAnnotations = (
    setOriginalAngle = false,
    shouldAddTapered = false
  ) => {
    const canvas = canvasInstance.current;
    if (!canvas) return;
  ...
  const changeCircleAngle = (newAngle) => {
    ...
    while (obj) {
      ...
      obj.circle2.crushFoldObject &&
        objectsToRotate.push(obj.circle2.crushFoldObject);
  ...
  const applyChange = () => {
    ...
    const snap = snapshotCircles(
      circles,
      "_undoPosition_resize",
      "_redoPosition_resize"
    );
  ...
  useDebouncedEffect(
    () => {
      if (
        ...
        isCircleSelected
          ? changeCircleAngle(objValue)
          : changeLineLength(objValue);
        canvasInstance.current.isChanged = true;

        const circles = canvasInstance.current
          .getObjects()
          .filter((o) => o.type === "circle");
