...
export default function useTapper() {
  ...
  function customizeAnnotationStyle(
    annoGroup,
    { stroke, fill, bgFill, textFill }
  ) {
  ...
  function removeLengthAnnotations(canvas) {
    const annos = canvas
      .getObjects()
      .filter((obj) => obj.type === "group" && obj._isMeasurement);
    //
  ...
  function handleLineSelection(line, isSelected) {
    ...
    const annotationColor = isSelected
      ? "#3355FF"
      : line.side === "A"
      ? "#E50000"
      : "#475569";
  ...
  const createAllSideAnnotations = (canvas) => {
    const sideLines = canvas
      .getObjects()
      .filter((obj) => obj.type === "line" && obj.side);
  ...
  useEffect(() => {
    ...
    allLines.forEach((line) => {
      ...
      line.originalBLength = line.bSideLineLength
    ...
    allLines.forEach((line) => {
      if (
        Math.round(calculateLineLength(line)) !==
          Math.round(line.bSideLineLength) &&
        line.bSideLineLength
      ) {
        canvas.setActiveObject(line.bSideLine);
    ...
    return () => {
      ...
      const bSideElements = canvas
        .getObjects()
        .filter((obj) => obj.side === "B");
      ...
      allLines.forEach((line) => {
        ...
        Math.round(calculateLineLength(line)) !==
        Math.round(calculateLineLength(line.bSideLine))
          ? (line.isTapered = true)
