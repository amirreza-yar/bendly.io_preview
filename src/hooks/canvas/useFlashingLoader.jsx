...
function sortFlashingNodes(flashing) {
  ...
  if (!startNode) {
    throw new Error(
      "No starting node found (missing a node without prev_node_id)."
    );
  }
...
function createCrushFoldObject(circle, direction, position) {
  ...
  if (position === "start") {
    ...
    crushFoldObject = new Circle({
      ...
      left:
        cx -
        6.75 * Math.cos(mainAngle - 90 * dirCoef) -
        2.6 * Math.cos(mainAngle),
      top:
        cy -
        6.75 * Math.sin(mainAngle - 90 * dirCoef) -
        2.6 * Math.sin(mainAngle),
      angle: 0,
...
export const addColorSideFlashing = (
  ...
  circles.map((circle) => {
    if (!circle.line1 && circle.line2) {
      ...
      startCrushFold &&
        canvas.add(createCrushFoldObject(circle, direction, "start"));
      ...
      const angle =
        prevAngle + (sign * Math.PI) / 2 + (angleRad - prevAngle) / 2;
      ...
      offset =
        ...
        Math.abs(
          Math.sin(
            (Math.atan2(
              circle.line2.y2 - circle.line2.y1,
              circle.line2.x2 - circle.line2.x1
            ) -
              Math.atan2(
                circle.line1.y1 - circle.line1.y2,
                circle.line1.x1 - circle.line1.x2
              )) /
              2
          )
        );
...
function createAndAddGradientsAndConnectingLines(
  ...
  const aLines = canvas
    .getObjects()
    .filter((obj) => obj.type === "line" && obj.side === "A");
  ...
  for (let i = 0; i <= aLines.length; i++) {
    ...
    if (drawConnectingLines) {
      const line = new Line(
        [points[i].x1, points[i].y1, points[i].x2, points[i].y2],
        {
          stroke: "#CBD5E1",
          strokeWidth: 1,
          selectable: false,
...
export function create3DFlashing(canvas, showAnnos = false) {
  ...
  const existingElements = canvas
    .getObjects()
    .filter(
      (obj) => obj.side === "B" || obj._isMeasurement || obj.type === "path"
    );
...
function addLengthAnnotation(
  canvas,
  line,
  side,
  negativeOffset,
  showTaperedFlag = true
) {
  ...
  objs
    ...
    .map((obj) => {
      if (obj.type === "line") {
        ...
        obj.set({
          height: obj.height / scale / 0.7,
          width: obj.width / scale / 0.7,
        });
