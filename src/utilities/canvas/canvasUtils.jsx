  ...
  objects.forEach((obj) => {
    if (obj.type === "line" || obj instanceof Line) {
      ...
      const newCenter = new Point(
        rotationPoint.x + rotatedX,
        rotationPoint.y + rotatedY
      );
...
export function createAngleAnnotationObj(ax, ay, px, py, bx, by, options = {}) {
  ...
  if (Math.abs(Math.abs(δ) - Math.PI / 2) < EPS) {
    ...
    const p_uv = new Point(
      px + (uNx + vNx) * markerLen,
      py + (uNy + vNy) * markerLen
    );
  ...
  function makeArrowLines(x, y, tangent) {
    ...
    return [
      new Line(
        [x, y, x + arrowLen * Math.cos(a1), y + arrowLen * Math.sin(a1)],
        {
          stroke: color,
          strokeWidth: lineStroke,
          strokeLineCap: "round",
      ...
      new Line(
        [x, y, x + arrowLen * Math.cos(a2), y + arrowLen * Math.sin(a2)],
        {
          stroke: color,
          strokeWidth: lineStroke,
          strokeLineCap: "round",
...
export function hasAnyOverlap(canvas, side = null) {
  ...
  if (side === "A") {
    ...
    lines = canvas
      .getObjects()
      .filter((o) => o.type === "line" && o.hitboxLine_B);
  } else {
    lines = canvas
      .getObjects()
      .filter((o) => o.type === "line" && o.hitboxLine);
